...
if (($_VALOR <> "") && ($_VENCIMENTO <> "") && ($_PARCELAATUAL <> "") && ($_PARCELAFINAL <> "")) {
    ...
    $Obj = new titulo($_IDEDITAR, $_PESSOA, $_VALOR, $_VENCIMENTO, $_PARCELAATUAL, $_PARCELAFINAL, 0, $_OBS, $_SITUACAO);

    $erro = $Controll->Save($Obj);
    if ($erro->erro) {
        echo $erro->mensagem;
    } else {
        echo '<META http-equiv="refresh" content="0;URL=?pag=' . $pag_titulospagar . '">';
    }
}
...
<script>
    function Cancelar() {
        window.location.href = location.href.replace(/\?.*/gi, "") + "?pag=<?php echo $pag_titulospagar ?>";
    }
</script>
