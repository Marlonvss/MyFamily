...
if ($_DESCRICAO <> "") {
    ...
    $Obj = new centrocusto($_IDEDITAR, $_DESCRICAO);

    $erro = $Controll->Save($Obj);
    if ($erro->erro) {
        echo $erro->mensagem;
    } else {
        echo '<META http-equiv="refresh" content="0;URL=?pag=' . $pag_centroscustos . '">';
    }
}

$Obj = new centrocusto($_IDEDITAR);
...
<script>
    function Cancelar() {
        window.location.href = location.href.replace(/\?.*/gi, "") + "?pag=<?php echo $pag_centroscustos ?>";
    }
</script>

<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Editar Centro de Custo</h1>
    </div>
</div>
